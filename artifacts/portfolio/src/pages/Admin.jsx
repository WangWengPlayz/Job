...
export default function Admin() {
  ...
  const [status, setStatus] = useState(getAvailability());
  const [jobType, setJobTypeState] = useState(getJobType());
  ...
  >
    ...
    className={`flex items-center gap-4 p-4 rounded-xl border-2 transition-all text-left ${status === "available" ? "border-green-500 bg-green-500/10" : "border-border bg-muted/30 hover:border-green-500/40"}`}
  ...
  >
              ...
              {status === "available" && <ToggleRight size={22} className="ml-auto text-green-500" />}
              {status !== "available" && <ToggleLeft size={22} className="ml-auto text-muted-foreground" />}
            </button>
    ...
    className={`flex items-center gap-4 p-4 rounded-xl border-2 transition-all text-left ${status === "busy" ? "border-rose-500 bg-rose-500/10" : "border-border bg-muted/30 hover:border-rose-500/40"}`}
  ...
  >
              ...
              {status === "busy" && <ToggleRight size={22} className="ml-auto text-rose-500" />}
              {status !== "busy" && <ToggleLeft size={22} className="ml-auto text-muted-foreground" />}
            </button>
          </div>
        </motion.div>
  ...
  >
    ...
    className={`p-4 rounded-xl border-2 transition-all text-left ${jobType === opt.value ? "border-primary bg-primary/10" : "border-border bg-muted/30 hover:border-primary/40"}`}
  ...
  >
          ...
          <div className="flex items-center gap-3 mb-6">
            <div
    className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-medium text-sm ${status === "available" ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-rose-500/10 text-rose-600 dark:text-rose-400"}`}
  >
              <span className="relative flex h-2 w-2">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${status === "available" ? "bg-green-500" : "bg-rose-500"}`} />
                <span className={`relative inline-flex rounded-full h-2 w-2 ${status === "available" ? "bg-green-500" : "bg-rose-500"}`} />
              </span>
              {status === "busy" ? "Currently Busy" : jobType === "fulltime" ? "Available \xB7 Full-Time" : jobType === "parttime" ? "Available \xB7 Part-Time" : "Available \xB7 Full-Time or Part-Time"}
            </div>
          </div>
  ...
  >
            {saved ? <><CheckCircle size={16} /> Saved Successfully!</> : <><ShieldCheck size={16} /> Save &amp; Apply to Portfolio</>}
          </Button>
