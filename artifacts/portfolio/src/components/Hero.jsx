...
export default function Hero() {
  ...
  return <section id="hero" className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-b from-transparent to-background/20 dark:to-background z-[-1]" />
      <div className="absolute top-1/4 -right-64 w-96 h-96 bg-primary/10 blur-[128px] rounded-full" />
      <div className="absolute bottom-1/4 -left-64 w-96 h-96 bg-blue-500/10 blur-[128px] rounded-full" />
  ...
  >
            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full font-medium text-sm mb-6 transition-colors ${avStatus === "available" ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-rose-500/10 text-rose-600 dark:text-rose-400"}`}>
              <span className="relative flex h-2 w-2">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${avStatus === "available" ? "bg-green-500" : "bg-rose-500"}`} />
                <span className={`relative inline-flex rounded-full h-2 w-2 ${avStatus === "available" ? "bg-green-500" : "bg-rose-500"}`} />
              </span>
            ...
            <div className="h-10 md:h-14 mb-6">
              <h2 className="text-2xl md:text-4xl font-semibold text-muted-foreground">
                {displayText}
                <span className="animate-pulse ml-1 inline-block w-[3px] h-[1em] bg-primary align-middle" />
              </h2>
            </div>
  ...
  >
            <div className="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96">
              <div className="absolute inset-0 bg-gradient-to-tr from-primary/30 to-blue-400/30 rounded-full animate-[spin_8s_linear_infinite] blur-2xl" />
